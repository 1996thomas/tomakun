...
import {
  clearKanaSession,
  loadSavedKanaSession,
  saveKanaSession,
} from "@/lib/kana-session.storage";
...
export default function KanaQuiz() {
  ...
  const [question, setQuestion] = useState(() => resumeSession?.question ?? generateKanaQuestion());
  ...
  function restoreSavedSession(): void {
    const saved = loadSavedKanaSession();
    if (!saved) return;
  ...
  useEffect(() => {
    if (shouldResume && resumeSession) clearKanaSession();
  ...
  useEffect(() => {
    if (sessionEnded) clearKanaSession();
  }, [sessionEnded]);

  useEffect(() => {
    const onResumeRequest = (event: Event) => {
      const custom = event as CustomEvent<{ module: "kana" }>;
      if (custom.detail?.module !== "kana") return;
      restoreSavedSession();
    };
  ...
  return (
    <section className="flex flex-1 flex-col justify-center">
      <div className="surface-card rounded-xl p-4 shadow-sm">
        ...
        <p className="text-muted text-center text-sm">{t("quiz.whatKatakana")}</p>
